          ...
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              <DialogTitle className="flex items-center justify-between">
                ...
                <span className="text-xl ml-2">{getStatusEmoji(selectedProject.status)}</span>
              </DialogTitle>
...
const ProjectCard = ({ project, onSelect }: { project: Project; onSelect: () => void }) => {
  return (
    <Card className="h-full overflow-hidden border border-border/50 hover:border-primary/50 transition-all duration-300 hover:shadow-md">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          {project.title}
          <span title={project.status} className="text-xl">
